...
class FavoriteDAO:
    ...
    def all(self, user_id, sort = 'title', order = 'ASC', limit = 6, skip = 0):
        ...
        def find_favorites(tx):
            rows = tx.run(f"""
                MATCH (u:User {{userId: $userId}})-[f:HAS_FAVORITE]->(m:Movie)
                RETURN m {{
                    .*,
                    favorite: true
                }} AS movie 
                ...
                LIMIT {limit}
            """, userId=user_id)
            return [row.value("movie") for row in rows]
        
        with self.driver.session() as session:
            return session.execute_read(find_favorites)
    # end::all[]
   ...
   *
    ...
    def add(self, user_id, movie_id):
        ...
        def add_to_favorites(tx):
            ...
            """
            return tx.run(query, userId=user_id, movieId=movie_id).single()

        with self.driver.session() as session:
            record = session.execute_write(add_to_favorites)
            if record:
                return record[0]
            else:
    ...
    def remove(self, user_id, movie_id):
        ...
        def remove_from_favorites(tx, user_id, movie_id):
            row = tx.run("""
                MATCH (u:User {userId: $userId})-[f:HAS_FAVORITE]->(m:Movie {tmdbId: $movieId})
                DELETE f
                RETURN m {
                    .*,
                    favorite: false
